# Specification Triage History

> Historical record only. This file captures the exploratory triage state before ADR D-021.
> It is not the current skip contract and must not be used to classify release readiness.
> Current executable dispositions live in `SpecDispositions.json`.
