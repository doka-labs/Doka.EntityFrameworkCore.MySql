    ...
    : AdHocJsonQueryRelationalTestBase
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public override Task Bad_json_properties_null_navigations(
        bool noTracking)
    ...
    [InlineData(false)]
    [InlineData(true)]
    public override Task Bad_json_properties_null_scalars(
        bool noTracking)
        => base.Bad_json_properties_null_scalars(noTracking);
...
""");
        ...
        if (!SpecTestTarget.IsEngineLimitationProbeEnabled())
        {
            return;
        }

        // These are the exact malformed documents from EF Core's SQL Server fixture. Native
        // MySQL and MariaDB JSON columns reject the first insert before materialization can
        // exercise the upstream null-property-name behavior. The probe condition keeps valid
        // bad-JSON contracts executable during ordinary runs while preserving a one-switch
        // re-evaluation path if an engine ever accepts this document shape.
        await ctx.Database.ExecuteSqlAsync(
            $$"""
INSERT INTO `Entities` (`Id`, `Scenario`, `OptionalReference`, `RequiredReference`, `Collection`)
VALUES(
