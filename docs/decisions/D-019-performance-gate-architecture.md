# D-019 -- Performance Gate Architecture

- **Status:** Implemented (gate infrastructure); CI activation deferred per re-evaluation trigger.
- **Date:** 2026-05-18
- **Scope:** `benchmarks/Doka.EntityFrameworkCore.MySql.Benchmarks/**` baseline-marked variants + `eng/check-benchmark-ratios.sh` gate script. CI workflow integration (`.github/workflows/benchmark.yml`, `.github/workflows/release-candidate.yml`) remains disabled.
...
| `JsonComparerBenchmark` | `NaiveJsonElementEqualsLoop` (per-call `GetRawText` + Ordinal string-compare) | `JsonElementEqualsLoop` (streaming `Utf8JsonReader` token-walk through `MySqlJsonValueComparers`) | bytes allocated per operation | `0.2` |
