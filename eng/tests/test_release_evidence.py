...
class ReleaseEvidenceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # The host may carry several SDKs. Release fixtures must inherit the
        # repository-approved exact SDK rather than ambient tool selection.
    ...
    def test_sdk_contract_accepts_one_exact_prerelease(self) -> None:
        """Permit prerelease selection only when the exact pin is prerelease."""
        path = self.repo / "global.json"
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["sdk"]["version"] = "11.0.100-rc.1.26425.128"
    ...
    def test_sdk_contract_rejects_prerelease_policy_mismatch(self) -> None:
        """Reject a prerelease pin that cannot select prerelease SDKs."""
        path = self.repo / "global.json"
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["sdk"]["version"] = "11.0.100-rc.1.26425.128"
        payload["sdk"]["allowPrerelease"] = False
        path.write_text(json.dumps(payload) + "\n", encoding="ascii")

        with self.assertRaisesRegex(
            release_evidence.EvidenceError,
            "must permit prerelease .NET SDK selection",
        ):
            release_evidence.approved_dotnet_sdk(self.repo)
                            ...
                            "topLevelPackages": [
                                {
                                    ...
                                    "resolvedVersion": "11.0.0-rc.1.26425.128",
                                },
                                {
